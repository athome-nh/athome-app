const Map<String, String> kur = {
  // choose_lan
  "Language": "زمان",
  "English": "ئینگلیزی",
  "Arabic": "عەرەبی",
  "Kurdish": "کوردی",
  "Get Start": "دەست پێبکە",

  // welcome_screen
  "wst01": "ئاسانتر و زیرەکانەتر  بازاڕ بکە",
  "wsd01": "ئەم پلاتفۆڕمە داهێنراوێکی نوێی دیزاین کراوەبۆ باشتر کردنی ئەزموونی بازاڕکردنی ئۆنلاین.",
  "wst02": "لە باشترینەکانەوە فێر ببە",
  "wsd02": "ئێمە ڕێنیشاندەری باشترین گەیاندن دەکەین لە کوردستان/ عێراق",
  "wst03": "گەیاندنی خێرا",
  "wsd03": "لە هەر کوێیەک بن بۆتان دەگەیەنین",
  "NEXT": "دواتر",
  "SKIP": "تێپەڕبوون",

  // nav_switch
  "Home": "سەرەکی",
  "Search": "گەڕان",
  "Cart": "سەبەتە",
  "Favorite": "دڵخوازه‌كان",
  "Account": "هەژمار",

  // Dialogbox --> nav_switch
  "Are you sure exiting the app": "دڵنیای لە چونە دەرەوە؟",
  "Yes": "بەڵێ",
  "No": "نەخێر",

  // home_page
  "Order now": "ئێستا داوا بكه‌",
  "Categories": "پۆلێنەکان",
  "Recent Order": "دوایین داواکاریەکان",
  "Discount": "داشکانەکان",
  "Highlight": "دیارترینەکان",
  "Best Sell": "باشترین فرۆش",
  "View All": "بینینی هەموو",

  // Dialogbox ( Register ) --> my_widget
  "Register First": "خۆت تۆمار بکە",
  "You need login": "پێویستە خۆت تۆمار بکەیت",
  "Register": "خۆتۆمارکردن",
  "Cancel": "پوچەڵ کردنەوە",

  // categories
  "All Categories": "گشت پۆلێنەکان",

  // all_item >> shtakan dobar boo

  // Search_page
  "Not found any item": "هیچ بەرهەمێک نەدۆزرایەوە",

  // my_cart
  "My Cart":"سەبەتەی بازاڕکردن",
  "Clear": "پاک کردنەوە",
  "Your cart is empty": "سەبەتەی بازاڕکردن بەتاڵە",
  "Delete": "سڕینەوە",
  "you can not add more this item": "ناتوانی زیاتر داوا بکەیت",
  "Sub Total": "کۆی گشتی",
  "Delivery Cost": "تێچوی گەیاندن",
  "Free Delivery": "گەیاندنی خۆڕایی",
  "Total": "کۆی گشتی",
  "Checkout": "دوپاتکردنەوە",
  "Clear cart":"لابردنی هەموو",
  "Are you sure you want to continue?":"دڵنیای له‌ به‌تاڵكردنه‌وه‌ی سه‌به‌ته‌ی كڕین؟",

  // favorite
  "No have any favorite": "بابەتی دڵخواز دیاری نەکراوە\nدڵخوازەکان دیاری بکە و جیاوازیەکان ببینە",

  // Page --> No Internet --> my_widget
  "no internet": "ئینتەرنێت بەردەست  نیە",

  // Page --> Login First --> my_widget
  "Please login first": "تکایە خۆت تۆمار بکە", // nmayaa

  // profilo
  "Image changed": "وێنەکە گۆڕدرا",
  "Information": "زانیاری",
  "Enter Name":"تۆمارکردنی ناو",
  "Enter Age":"تۆمارکدنی تەمەن",
  "Edit":"ڕێکخستن",
  "Orders":"داواکاریەکانم",
  "Address": "ناونیشان",
  "About us":"دەربارەی ئێمە",
  "Log out":"چوونە دەر",
  "Save": "پاشەکەوت کردن",

  // order_screen
  "My Orders":"داواکاریەکانم",
  "On going": "بەردەوامی هەیە",
  "History":"داواکاریەکانی ڕابردوو",
  "Order number:":"ژمارەی داواکاری",
  "Date:": "به‌روار:",
  "You not have any order": "هیچ داواکاریەکت نیە",
  "Re order":"دوبارە داواکردن",
  "View":"پیشاندان",

  // order_items
  "Past Order": "داواکاریێشوتر",
  "Add More Items": "زیادکردنی بەرهەمی تر",

  // old_order
  "Quantity: ":"بڕی",
  "Date and Time":"کات و بەروار",
  "Order No.":"ژمارەی داواکاری",

  // check_out
  "Delivery Address": "ناونیشانی گەیاندن",
  "Not have any location": "هیچ ناونیشانێک تۆمار نەکراوە",
  "Add new Location":"زیاد کردنی ناونیشانی گەیاندن",
  "Pyment Method": "شێوازی پارەدان",
  "Cash on delivery": "پارەدان لەکاتی گەیاندن",
  "Coming soon": "بەم زوانە",
  "Note": "تێبینی",
  "Add your note": "زیادکردنی تێبینی",
  "Thank You!": "سوپاس",
  "for yor order": "بۆ بازاڕکردن و هەڵبژاردنی دڵی لاس مارکێت",
  "Order Number:": "ژمارەی داواکاری:",
  "YourOrderIsNowBeingProcessed": "داواکاریەکەت لە جێبەجێ کردن دایە. لە کاتی وەرگرتنی داواکاریەکەت ئاگادار دەکرێیتەوە.دەتوانیت ئاگاداری داواکاریەکەت ببیت",
  "Track My Order": "بینینی ئامادەکاری گەیاندنەکەم",
  "Back to Home": "گەڕانەوە بۆ بازاڕکردن",
  "Please Delivery Address":"تکایە ناونیشانی گەیاندن زیاد بکە",
  "Send Order": "داواکاریەکەت بنێرە",
  "sorryWeDoNotHaveDelivery": "ببورن، لە ئیستا گەیاندمان بەردەست نیە هەوڵ دەدەین لە ڕۆژی داهاتوو گەیاندن ئەنجام بدەین لە زووترین کاتدا. هەرەوەها دەتوانن داواکاریەکەتان بگەڕێننەوە پێش قبوڵ کردنی لەلایەن سەرپەرشتیار",
  "OK": "دڵنیام",

  // login_page
  "Wellcome back": "بەخێر بێیتەوە",
  "EnterYourPhoneNumberToShop": "ژمارە مۆبایل داخڵ بکە بۆ ئەوەی ئاسانتر و زیرەکانەتر بازاڕ بکەیت.",
  "Please enter your phone number": "ژمارە مۆبایل داخڵ بکە ",
  "Please enter your phone number correct":"تکایە ژمارە مۆبایل بە دروستی داخڵ بکە",
  "Please remove 0 form start": "سفری سەرەتا بسڕەوە",
  "Phone Number": "ژمارە مۆبایل",
  "WeWillSendYouOTP":"کۆدی نهێنی یەک جارە دەنێرین بۆ ژمارە تەلەفونەکەت تکایە خۆت تۆمار بکە.",

  // locationdeatil
  "Locations": "ناونیشان",
  "Name": "ناو",
  "Add name": "زیادکردنی ناو",
  "Add name location": "زیادکردنی ناوی ناونیشان",
  "Area": "ناوچە",
  "Add Area": "زیادکردنی ناونیشان",
  "Type Building": "جۆر",
  "Floor number": "ژمارە",
  "Enter floor number": "ژمارەی خانوو",
  "building name/number": "خانوی ژمارە",
  "Enter building name/number": "ناو/ژمارە ی خانوو داخڵ بکە",
  "number": "ژمارە",
  "Enter number": "ژمارەکە داخڵ بکە",
  "Phone number": "ژمارەی مۆبایل",
  "Add phone number": "زیادکردنی ژمارەی مۆبایل",
  "Delete location success": "سڕینەوەی ناونیشان ئەنجام درا",

  // singin_up
  "Complete Account": "هەژمارەکەت دروست کرا",
  "Enter your account information to complete your account":"زانیاری هەژمارەکەت داخڵ بکە",
  "Enter your Name": "ناوت داخڵ بکە",
  "Enter your full name": "ناوی تەواوت داخڵ بکە",
  "City": "شار",
  "Age": "تەمەن",
  "unknown occurred error please try again later":"هەڵەیەك تۆمارکرا ، تکایە دواتر هەوڵ بدەوە",
  "Confirm": "دڵنیابوونەوە",

  // verification
  "Hold till the waiting time ends":"دوبارە بکەوە لە",
  "Please enter code":"تکایە کۆدی یەك جارە داخڵ بکە",
  "Verification": "دڵنیابوونەوە",
  "Please enter the 6 digit code sent to": "تکایە کۆدی ٦ ژمارەیی داخڵ بکە",
  "Don't resive the OTP ?": "کۆدی یەک جارەییت بە دەست نەگەیشت ?",
  "Try again in": "دوبارە داوابکەوە لە ",
  "Resend": "دووبارە ناردنەوە",
  "Verify": "دڵنیا بوونەوە",
  "Account Disabled": "هەژمارەکە ڕاگیرا",
  "Account is disable please contact athome admin":"هەژمارەکە ڕاگیرا ، پەیوەندی بە سەرپەرشتیاری دڵی لاس بکە",
  "the code is un correct": "کۆدەکە هەڵەیە",

  // map
  "Map": "نەخشە",
  "Zoom in Please": "نزیکتر بکەوە",
  "Sorry, we don't deliver here":"ببورە، گەیادنمان بۆ ناوچەی ناوبراو بەردەست نیە",
  "Delivery To:": "گەیاندن بۆ:",
  "try again": "دوبارە هەوڵ بدەوە",
  "Failed to load data": "سەرکەوتوو نەبوو لە پیشاندانی داتا",

  // track_order
  "Track Order": "بینینی داواکاری",
  "Order:": "داواکاری:",
  "Cancel order": "پوچەڵ کردنەوەی",
  "View order":"بینینی داواکاری ",

  // about_screen
  "WelcomeToAthomeOnlineMarketApplication!": "بەخێربێن بۆ ئەپلیکەیشنی ئۆنلاینی دڵی لاس! ئێمە لێرەین بۆ ئەوەی ڕێگایەکی گونجاو و کاریگەرتان بۆ دابین بکەین بۆ کڕینی کەلوپەلەکانتان و گەیاندنیان بۆ دەرگای ماڵەکانتان. پلاتفۆرمەکەمان دیزاین کراوە بۆ باشترکردنی ئەزموونی کڕینەکانت و کڕینی کاڵا بە خۆڕایی. کورته یه ک له سه ر چۆنیه تی به کارهێنانی",
  "WideSelectionOfProductsOurOnlineSupermarket": "هەڵبژاردنی فراوان لە بەرهەمەکان:  دڵی لاس مارکێتی ئۆنلاین کۆمەڵێک بەرهەمی فراوان پێشکەش دەکات، لە بەرهەمی تازەوە بۆ کەلوپەلی پێویست، کەلوپەلی ماڵەوە، بەرهەمەکانی پێداویستی کەسی و هتد.... دەتوانیت بە ڕێگای جۆراوجۆردا بگەڕێیت و هەموو ئەو شتانە بدۆزیتەوە کە پێویستتە لە یەک شوێندا.",
  "EasyToUseInterfaceOurUserFriendly": "ڕووکاری ئاسان بۆ بەکارهێنان: ڕووکاری ئاسانی بەکارهێنەرمان دیزاین کردوە بۆ ئەوەی ئەزموونی کڕینەکەت ئاسان و چێژبەخش بێت. دەتوانیت بە ئاسانی بەدوای بەرهەمەکاندا بگەڕێیت، زیادیان بکەیت بۆ سەبەتەی کڕین، و بەردەوام بیت لە بە دوا داچوونی داواکاریەکانت بە چەند کلیکێک.",
  "SecurePaymentWePrioritizeTheSecurity": "شێوازی پارەدانی پارێزراو: ئێمە لە دڵی لاس جەختمان لە شێوازی پارەدانێکی پاریزراو کردۆتەوە کە لەڵ ویست و پارێزراوی ئێوە بگونجێت و بە ئەرکی گرنگ و ئەولەویمان داناوە بۆ ئەوەی بە شێوازێکی پارێزراو بازاڕیەکانتان ئەنجام بدەن.",
  "PromotionsAndDealsKeep": "داشکان و مامەڵەکان: ئاگاداری بابەتە تایبەت و داشکاندن و مامەڵەکان بن لەسەر پلاتفۆرمەکەمان. ئێمە دەمانەوێت یارمەتیت بدەین لە پاشەکەوتکردن لە کاتی کڕیندا.",
  "CustomerSupportIfYouHaveAnyQuestions": "خزمەتگوزاری کەسی: بەشی خزمەتگوزاری کڕیارانمان بەردەوام هاوکار دەبن لە کاتی هەبوونی هەر پرسیار و پێشنیارێک. دوو دڵ مەبە و تیمی دڵی لاس لەگەڵتدایە",
  "FeedbackAndImprovementWeValueYourFeedback": "پێشنیار و بەرەوپێشچوون: ئێمە بەهای کاردانەوەکەت دەزانین. پێشنیارەکانت یارمەتیمان دەدەن بۆ باشترکردنی پلان و خزمەتگوزاریەکانمان. ئێمە هەمیشە بەدوای ڕێگایەکدا دەگەڕێین بۆ باشترکردن و پێشکەشکردنی باشترین ئەزموون.",
  "ThankYouForChoosingOurAthome": "سوپاس بۆ هەڵبژاردنی دڵی لاس، بازاڕ کردنت ئەنجام بدە و بەدواداچوون دەکرێت بۆ داواکاریەکەت لەلایەن ئێمەوە. دڵ خۆش بە بازاڕکردنت لەگەڵ دڵی لاس.",
  "About Us": "دەربارەی ئێمە",

  //more old_order
  "Order Placed":"داواکاریەکەت تۆمارکرا",
  "Processing Order":"لە ژێر کارکردن دایە",
  "Order Is On way":"داواکاریەکە لە ڕێگایە ",
  "Order Ready For Pickup":"داواکاریەکە ئامادەیە بۆ وەرگرتن",
  "Order is delivered":"داواکاریەکە گەیەنرا",
  "Undelivered":"داواکاریەکە نەگەیەنرا",
  "Order Deatil":"زانیاری داواکاریەکە", 

  //
  "min title":"کەمترین داواکاری پێشکەشکراو",
  "min deatil":"تکایە بەرهەمی زیاتر لە سەبەتەی کرین داوابکە، بۆ ئەوەی داواکاریەکەت جێ بەجێ بکرێت",

  // item_categories
  "You not have any item":"هیچ بابەتێک بەردەست نیە",

  // oneitem --> labir kraboo
  "Add To Cart":"زیادی بکە بۆ سەبەتە",
};